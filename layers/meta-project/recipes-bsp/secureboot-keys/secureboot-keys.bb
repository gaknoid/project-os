...
SECUREBOOT_PK_CN ?= "Platform Key"
SECUREBOOT_KEK_CN ?= "Key Exchange Key"
SECUREBOOT_DB_CN ?= "Signature Database"
SECUREBOOT_DBX_CN ?= "Revoked Signature Database"
...
do_compile() {
    ...
    create_signing_cert dbx "${SECUREBOOT_DBX_CN}"
    ...
    cert-to-efi-sig-list -g "${OWNER_GUID}" "${SECUREBOOT_KEYS_OUTDIR}/dbx.crt" "${SECUREBOOT_KEYS_OUTDIR}/dbx.esl"
    ...
    sign-efi-sig-list -k "${SECUREBOOT_KEYS_OUTDIR}/KEK.key" -c "${SECUREBOOT_KEYS_OUTDIR}/KEK.crt" dbx "${SECUREBOOT_KEYS_OUTDIR}/dbx.esl" "${SECUREBOOT_KEYS_OUTDIR}/dbx.auth"
}
