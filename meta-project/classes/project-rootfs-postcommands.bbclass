...
ROOTFS_POSTUNINSTALL_COMMAND:sota += "remove_rpm_dnf_data_from_image; "
...
ROOTFS_POSTUNINSTALL_COMMAND:sota += "remove_deb_apt_data_from_image; "
...
ROOTFS_POSTUNINSTALL_COMMAND:sota += "remove_ipk_opkg_data_from_image; "

# Save a factory copy of /boot in the image and add a tmpfiles rule
# to restore it on systems where /boot starts uninitialized.
install_factory_boot_seed() {
    install -d ${IMAGE_ROOTFS}${datadir}/factory
    rm -rf ${IMAGE_ROOTFS}${datadir}/factory/boot
    cp -a ${IMAGE_ROOTFS}/boot ${IMAGE_ROOTFS}${datadir}/factory/boot

    install -d ${IMAGE_ROOTFS}${nonarch_libdir}/tmpfiles.d
    cat > ${IMAGE_ROOTFS}${nonarch_libdir}/tmpfiles.d/10-populate-boot.conf << EOF
# Automatically propagate all /boot content from ${datadir}/factory/boot;
# C+! /boot - - - - -
EOF
}

ROOTFS_POSTUNINSTALL_COMMAND += "install_factory_boot_seed; "
