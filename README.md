...
```bash
cd <project-root>
repo init -u http://github.com/gaknoid/project-os -m manifests/scarthgap-head.xml -b main
ln -sfr .repo/manifests/.gitignore .gitignore
repo sync -j$(nproc)
ln -sfr .repo/manifests layers/project-os
ln -sfr .repo/manifests/.vscode .vscode
```
...
cd <project-root>
rm -rf build/conf
TEMPLATECONF=$(pwd)/.repo/manifests/layers/meta-project/conf/templates/default \
